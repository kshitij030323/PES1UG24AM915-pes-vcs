...
int index_load(Index *index) {
    ...
    if (!f) return 0; // Missing file means empty index, which is not an error
...
int index_save(const Index *index) {
    ...
    *sorted = *index;
    ...
    char temp_path[512];
    snprintf(temp_path, sizeof(temp_path), "%s.tmp_%d", INDEX_FILE, getpid());
